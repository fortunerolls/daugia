...
(function () {
  ...
  function buildWlRow(id, addr) {
    ...
    btn.style.padding = "8px 14px";
          ...
          // Nút "Mở" — đọc UNC đa nguồn + fallback hỏi dán UNC + mở tương thích mobile
    btn.onclick = async () => {
      const key = `unc:${id}:${addr.toLowerCase()}`;
      let unc = null;

      // 1) Đọc UNC đã lưu (nếu có)
      try {
        unc = localStorage.getItem(key) || sessionStorage.getItem(key);
      } catch (_) {}

      // 2) Fallback: nếu chưa có UNC trên thiết bị này, hỏi người dùng dán link UNC
      if (!unc) {
        const ask = prompt(
          "Chưa có link UNC trên thiết bị này.\nHãy dán link UNC (https://...) để mở và lưu lại:",
          ""
        );
        if (ask && /^https?:\/\//i.test(ask.trim())) {
          unc = ask.trim();
          try {
            localStorage.setItem(key, unc);
            sessionStorage.setItem(key, unc);
          } catch (_) {}
        }
      }

      // 3) Mở bằng <a> tạm để tương thích MetaMask/Safari iOS
      if (unc && /^https?:\/\//i.test(unc)) {
        const a = document.createElement("a");
        setSmartLink(a, unc); // tự động chọn _self (mobile) / _blank (desktop)
  ...
  function makeUpdateForm(cardNode, id, cutoffSec) {
    ...
    btnSubmit.onclick = async (e) => {
      ...
      try {
        ...
        try {
          if (unc) {
            const k = `unc:${id}:${addr.toLowerCase()}`;
            sessionStorage.setItem(k, unc);
            localStorage.setItem(k,  unc);   // thêm dòng này để thiết bị hiện tại lưu bền
          }
        } catch (_) { /* noop */ }
